...
exports.PostCarUsers = async (req, res) => {
  try {
    const { username, email, age, gender, dob, city, profession, password } =
      req.body;
...
exports.EditCarUsers = async (req, res) => {
  try {
    ...
    const carUser = await CarUser.findByIdAndUpdate(
      req.params.id,
      updateFields,
...
exports.PatchCarUsers = async (req, res) => {
  try {
    ...
    const carUser = await CarUser.findByIdAndUpdate(req.params.id, updates, {
      new: true,
      runValidators: true,
    });
    if (!carUser) return res.status(404).json({ message: "User not found" });
